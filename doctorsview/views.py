...
import json
import uuid
from modelsview.models import HospitalAccounts
from patientsview.models import PatientAccount
...
class HospitalTotalView(APIView):
    def get(self,request):
        totalDoctors=DoctorAccount.objects.filter(hospital_id=request.GET['hospital_id']).count()
        verifiedDoctors=DoctorAccount.objects.filter(email_Verified=True).count()
        notVerifiedDoctors=DoctorAccount.objects.filter(email_Verified=False).count()
        patientIDCreated=PatientAccount.objects.filter(account_created_hospital_id=request.GET['hospital_id']).count()
        response=Response()
        response.data= {
            "total_doctors":totalDoctors,
            "verified_doctors":verifiedDoctors,
            "not_verified_doctors":notVerifiedDoctors,
            'total_patient_id_created':patientIDCreated
        }
        return response
